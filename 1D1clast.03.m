...
% Should only change dyclast, clastnum, tclasttwo and tclastthree
...
tclastthree = 10.; % (s) Time when a third clast is deposited


% Define model parameters
% Naming scheme: y is a location. dy is a thickness. 
% Node is nodal location (when combined with y) and number of nodes (when
% combined with dy) 
...
dystep = dysystem/(dysystemnode-1); % (m) Length one grid step (dy)
...
yclasttwotop = (yclastonetop + dyclast); % (m) Boundary between the top of the second clast and the air (if spatter pile has only two clasts)
yclasttwotopnode = (yclastonetopnode + dyclastnode); % Node of the boundary between the top of the second clast and the air (if spatter pile has only two clasts)
yclastthreetop = (yclasttwotop + dyclast); % (m) Boundary between the top of the third clast and the air (if spatter pile has only three clasts)
yclastthreetopnode = (yclasttwotopnode + dyclastnode); % Node of the boundary between the top of the third clast and the air (if spatter pile has only three clasts)
...
if(y(i)> ypilebottom && y(i)<= yclastonetop)
    T0imp(i) = Tclast; % If we are located within the clast (including bottom node and top node of the clast) our initial temp is now 1373K
end 
end 
...
L = sparse(dysystemnode,dysystemnode); % The lefthand side of the matrix is 801 nodes by 801 nodes and is mostly made up of zeros (sparse)
R = zeros(dysystemnode,1); % The righthand side of the matrix is the vector and it is 801 nodes by 1
