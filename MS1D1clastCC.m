% 1D 1 clast conduction and convection
...
% Thermal constants
kground = 0.3601; % (W/(mK)) Thermal conductivity of the ground (previously deposited basalt)
kclast = 0.3601; % (W/(mK)) Thermal conductivity of the clast (basalt)
kair = 0.0264; % (W/(mK)) Thermal conductivity of the air
cpground = 1490.; % (J/(kgK)) Specific heat
cpclast = 1490.;
cpair = 1006.;
rhoground = 850.; % (kg/m3) Density 
rhoclast = 850.;
rhoair = 1.204; 
...
dt = dtexp./100; % (s) Length of one timestep
 
% Things to vary with different model runs
 
tnum = 1000000; % Number of timesteps (How many timesteps do you want to run the code for?)
...
for t=1:1:tnum
    ...
    for i=ypilebottomnode+2:1:yclastonetopnode-1
        ...
        L(yclastonetopnode,yclastonetopnode) = (kclast/(rhoclast*cpclast*(dystep^2))) + ((h)/(rhoclast*cpclast*dystep)) + (1/dt);
        R(yclastonetopnode,1) = Ti(yclastonetopnode)/dt + ((Tair*h)/(rhoclast*cpclast*dystep));
