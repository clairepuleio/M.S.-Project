% 1D multiple clasts conduction only
...
tnum = 500000; % Number of timesteps (How many timesteps do you want to run the code for?)
...
for t=1:1:tnum
    ...
    for i=2:1:ypilebottomnode
        ...
        % Define internal nodes of the clast one (conduction only)
    for i=ypilebottomnode+2:1:yclastonetopnode-1
        L(i,i-1) = -kclast/(rhoclast*cpclast*(dystep^2));
        L(i,i) = (kclast/(rhoclast*cpclast*(dystep^2))) + (kclast/(rhoclast*cpclast*(dystep^2))) + 1/dt;
        L(i,i+1) = -kclast/(rhoclast*cpclast*(dystep^2));
        ...
        % Define clast one/clast two boundary
         L(yclastonetopnode,yclastonetopnode-1) = -kclast/(rhoclast*cpclast*(dystep^2));
        L(yclastonetopnode,yclastonetopnode) = (kclast/(rhoclast*cpclast*(dystep^2))) + (kclast/(rhoclast*cpclast*(dystep^2))) + 1/dt;
        L(yclastonetopnode,yclastonetopnode+1) = -kclast/(rhoclast*cpclast*(dystep^2));
        R(yclastonetopnode,1) = Ti(yclastonetopnode)/dt;
        
        
        
        % Define internal nodes clast two
        for i=yclastonetopnode+1:1:yclasttwotopnode-1
        L(i,i-1) = -kclast/(rhoclast*cpclast*(dystep^2));
        L(i,i) = (kclast/(rhoclast*cpclast*(dystep^2))) + (kclast/(rhoclast*cpclast*(dystep^2))) + 1/dt;
        L(i,i+1) = -kclast/(rhoclast*cpclast*(dystep^2));
        R(i,1) = Ti(i)/dt;
        ...
        L(yclasttwotopnode,yclasttwotopnode-1) = -kclast/(rhoclast*cpclast*(dystep^2));
        L(yclasttwotopnode,yclasttwotopnode) = (kclast/(rhoclast*cpclast*(dystep^2))) + (kair/(rhoclast*cpclast*(dystep^2))) + 1/dt;
        L(yclasttwotopnode,yclasttwotopnode+1) = -kair/(rhoclast*cpclast*(dystep^2));
        R(yclasttwotopnode,1) = Ti(yclasttwotopnode)/dt;
    ...
    for i=ypilebottomnode+2:1:yclastonetopnode-1
        ...
        % Define clast two/clast three boundary
        L(yclasttwotopnode,yclasttwotopnode-1) = -kclast/(rhoclast*cpclast*(dystep^2));
        L(yclasttwotopnode,yclasttwotopnode) = (kclast/(rhoclast*cpclast*(dystep^2))) + (kclast/(rhoclast*cpclast*(dystep^2))) + 1/dt;
        L(yclasttwotopnode,yclasttwotopnode+1) = -kclast/(rhoclast*cpclast*(dystep^2));
        R(yclasttwotopnode,1) = Ti(yclasttwotopnode)/dt;
        
        % Define internal nodes of clast three
        for i=yclasttwotopnode+1:1:yclastthreetopnode-1
        L(i,i-1) = -kclast/(rhoclast*cpclast*(dystep^2));
        L(i,i) = (kclast/(rhoclast*cpclast*(dystep^2))) + (kclast/(rhoclast*cpclast*(dystep^2))) + 1/dt;
        L(i,i+1) = -kclast/(rhoclast*cpclast*(dystep^2));
        R(i,1) = Ti(i)/dt;
        ...
        L(yclastthreetopnode,yclastthreetopnode-1) = -kclast/(rhoclast*cpclast*(dystep^2));
        L(yclastthreetopnode,yclastthreetopnode) = (kclast/(rhoclast*cpclast*(dystep^2))) + (kair/(rhoclast*cpclast*(dystep^2))) + 1/dt;
        L(yclastthreetopnode,yclastthreetopnode+1) = -kair/(rhoclast*cpclast*(dystep^2));
        R(yclastthreetopnode,1) = Ti(yclastthreetopnode)/dt;
...
if(timesum>(tclasttwo-dt) && timesum<(tclasttwo) && jstep<clastnum)
     Ti(yclastonetopnode+1:yclasttwotopnode)= Tclast;
end
...
if(timesum>(tclastthree-dt) && timesum<(tclastthree) && jstep<clastnum)
     Ti(yclasttwotopnode+1:yclastthreetopnode)= Tclast;
end
